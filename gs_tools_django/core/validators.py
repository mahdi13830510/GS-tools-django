...
def is_valid_throttle_rate(rate):
    # Regular expression to match the throttle rate pattern
    pattern = re.compile(r"^(\d+)/((s|sec|m|min|h|hour|d|day))$")

    # Check if the rate matches the pattern
    if not pattern.match(rate):
        msg = f"Incorrect throttling rate value: {rate}"
        raise Exception(msg)
